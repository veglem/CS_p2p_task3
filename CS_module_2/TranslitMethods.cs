// Данный класс был нагло украден вот отсюда: https://babakov.net/blog/netcore/331.html
// Это не критичный компонент системы, но делает её законченной и логичной
// Весь код был осознан, поэтому готов к вопросам по этому блоку
namespace Translit
{
    public class TranslitMethods
    {
        public class Translitter
        {
            ...
            public string Translit(string source, TranslitType type)
            {
                ...
                {
                    result += (TranslitSymbols.FirstOrDefault(x => x.SymbolRus == source[i].ToString() && x.TranslitType==type) == null
                        ? source[i].ToString()
        ...
        private class TranslitSymbol
        {
            ...
            public string SymbolRus { get; set; } = null!;
            public string SymbolEng { get; set; } = null!;
        }
    }
}